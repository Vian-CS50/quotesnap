    ...
    <div className="w-full">
      ...
      >
        <span className="flex items-center gap-2 text-[#1C1917]">
          <Calculator className="w-4 h-4 text-[#A8A29E] group-hover:text-[#C2410C] transition-colors" />
          <span className="font-medium">Material Calculator</span>
        </span>
        {open ? <X className="w-4 h-4 text-[#A8A29E]" /> : <span className="text-xs text-[#A8A29E]">Turf · Pavers · Concrete · Mulch · Wall · Fence</span>}
      </button>
            ...
            <div className="pt-3 space-y-3">
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                      calcType === type
                        ...
                        : "bg-white-border/30 text-[#A8A29E] hover:text-[#1C1917]"
                    }`}
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {config.fields.map((field) => (
                  <div key={field.key}>
                    <label className="block text-[10px] uppercase tracking-wider text-[#A8A29E] mb-1">
              ...
              {results.some((r) => parseFloat(r.value) > 0) && (
                <div className="p-3 rounded-sm bg-orange-50 border border-[#C2410C]/20 space-y-1.5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold text-[#C2410C]">{config.title}</span>
                    <button
                      onClick={copyResults}
                      className="text-xs text-[#A8A29E] hover:text-[#1C1917] inline-flex items-center gap-1 transition-colors"
                    >
                  ...
                  {results.map((r) => (
                    <div key={r.label} className="flex justify-between text-sm">
                      <span className="text-[#A8A29E]">{r.label}</span>
                      <span className="font-medium text-[#1C1917]">
                        {r.value} <span className="text-xs text-[#A8A29E]">{r.unit}</span>
                      </span>
                    </div>
                  ))}
                </div>
